...
const LightboxAddRank = () => {
    ...
    const {handleShowLightBox, clickNumber} = useContext(Context) //解構賦值Context內的物件資料
    ...
    return (
        <div className='blackBackGround'>
            <div className="inSideBox">
                <div>
                    <span className="cancelBtn" onClick={handleShowLightBox}>X</span>
                </div>
                <div>大俠貴名</div>
                <input type="text" onChange={(e)=>handleKeyName(e)}/>
                <div>嗆聲留言</div> 
                <input type="text" onChange={(e)=>handleKeyComment(e)}/>
                <button onClick={addToRank} className='addToRankBtn'>add To Rank</button>
            </div>
            <style jsx>{`
                *{
